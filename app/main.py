...
async def _ingest_pdf(pdf_bytes: bytes, filename: str, allow_existing: bool = True,
    ...
    for pi, page in enumerate(doc.pages):
        try:
            ...
            # 图纸过复杂导致token超长，尝试缩小图片重试
            import logging as _log
            _log.getLogger("pdfsim").warning("页面%d BGE编码失败(%s), 尝试缩小重试", pi+1, e)
            try:
                small_img = page.image.resize(
                    tuple(s // 2 for s in page.image.size),
                    getattr(page.image, 'LANCZOS', 1)
                )
                vec = fe.extract(small_img, text=full_text[:500])
                vectors.append(vec)
            except Exception as e2:
                _log.getLogger("pdfsim").warning("页面%d 缩小后仍失败(%s), 跳过", pi+1, e2)
                continue
    if not vectors:
        raise HTTPException(status_code=400, detail="PDF所有页面均无法编码(图纸过于复杂)")
    ...
    info = DocRecord(
        ...
        phash=fe.file_signature(doc.pages[0].image)[:16] if doc.pages else "",
        signature=first_sig,
